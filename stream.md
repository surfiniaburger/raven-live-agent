...
"Hello!"
"Sure! Japan's history is a..." (partial content)
"Ah, wait."
"OK, how can I help?" [interrupted: true]
Difference from Other Streaming Types¶
...
Audio+video: 2 min
With Part 4: Context Window Compression: Unlimited	Both: 10 min
With Part 4: Context Window Compression: Unlimited
...
Official Documentation: Gemini Live API Guide | Vertex AI Live API Overview
...
App-level Session Persistence	❌ Manual implementation	✅ SQL databases (PostgreSQL, MySQL, SQLite), Vertex AI, in-memory (see ADK Session docs)
Platform Flexibility¶
...
yield Event
Web / Mobile
WebSocket / SSE (e.g. FastAPI)
LiveRequestQueue
Runner
Agent
LLM Flow
GeminiLlmConnection
Gemini Live API / Vertex AI Live API
...
Create a SessionService: for getting or creating ADK Session
Create a Runner: for providing a runtime for the Agent
...
Create a LiveRequestQueue for sending user messages to the Agent
...
Client
Phase 1: Application Initialization (Once at Startup)
Phase 2: Session Initialization (Every Time a User Connected)
Phase 3: Bidi-streaming with run_live() Event Loop
par
[Upstream: User sends messages via
LiveRequestQueue]
[Downstream: Agent responds via Events]
(Event loop continues until close signal)
Phase 4: Terminate Live API session
1. Create Agent(model, tools, instruction)
2. Create SessionService()
3. Create Runner(app_name, agent, session_service)
1. WebSocket connect(user_id, session_id)
2. get_or_create_session(app_name, user_id, session_id)
3. Create RunConfig(streaming_mode, modalities)
4. Create LiveRequestQueue()
5. Start run_live(user_id, session_id, queue, config)
Connect to Live API session
User message (text/audio/video)
send_content() / send_realtime()
Buffered request
Process request
Stream to Live API
Streaming response
Process response
yield Event (text/audio/tool/turn)
...
Demo implementation: agent.py:10-15
...
Demo implementation: main.py:37
...
Demo implementation: main.py:50,53
...
Demo implementation: main.py:155-161
...
Demo implementation: main.py:110-124
...
Demo implementation: main.py:163
...
Demo implementation: main.py:169-217
...
Demo implementation: main.py:253
...
Demo implementation: main.py:238-253
...
Persistent session services: Consider using persistent session services (DatabaseSessionService or VertexAiSessionService). See the ADK Session Services documentation for more details.
1.6 What We Will Learn¶
...
Next: Part 2: Sending Messages with LiveRequestQueue →
 Back to top
Previous
ADK Gemini Live API Toolkit
Next
